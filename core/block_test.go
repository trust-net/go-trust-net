...
func TestNewSimpleBlock(t *testing.T) {
	...
	block := NewSimpleBlock(newMySimpleHeader("previous"), newMySimpleHeader("genesis"), now, &myNode{})
	if block.Previous().String() != "previous" {
		t.Errorf("Block header does not match: Expected '%s', Found '%s'", "previous", block.Previous())
	}
	if block.Genesis().String() != "genesis" {
		t.Errorf("Block header does not match: Expected '%s', Found '%s'", "genesis", block.Genesis())
	}
	if block.Miner().Id() != "test node" {
		t.Errorf("Block minder does not match: Expected '%s', Found '%s'", "test node", block.Previous())
	}
	if block.Nonce() != nil {
...
func TestSimpleBlockHash(t *testing.T) {
	...
	data := ""
	data += block.previous.String() + block.miner.Id() + block.genesis.String() + fmt.Sprintf("%d",block.td)
